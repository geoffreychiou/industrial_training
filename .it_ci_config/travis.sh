...
export CI_PARENT_DIR=.it_ci_config  # This is the folder name that is used in downstream repositories in order to point to this repo.
export HIT_ENDOFSCRIPT=false
export REPOSITORY_NAME=${PWD##*/}
export CATKIN_WS=/root/ws_training
...
if ! [ "$IN_DOCKER" ]; then
    ...
    case "$ROS_REPO" in
        ros-shadow-fixed)
            export DOCKER_IMAGE=industrial_training/industrial_training:$ROS_DISTRO-ci-shadow-fixed
            ;;
        *)
            export DOCKER_IMAGE=industrial_training/industrial_training:$ROS_DISTRO-ci
            ;;
    esac
